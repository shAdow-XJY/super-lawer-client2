<template>
 	<view v-if="update">
 				...
 				<view class="message">
 					<view v-if="buttonShow">
 						...
 						<u-cell-group :border="false">
 							<u-cell-item bg-color="#ffffff" title="项目名称" :value="project_detail.project_name"
 								:arrow="false" class="cell"></u-cell-item>
 						</u-cell-group>
 					</view>
 					<u-cell-group v-else>
 						<u-cell-item bg-color="#ffffff" title="项目名称" :value="project_detail.project_name"
 							:arrow="false" class="cell"></u-cell-item>
 						<u-cell-item bg-color="#ffffff" title="项目类型" :value="project_detail.project_type"
 							:arrow="false" class="cell"></u-cell-item>
 						<u-cell-item bg-color="#ffffff" title="支付情况"
 							:value="!project_detail.pay_picture_url?'未支付':project_detail.is_payment?'已支付':'审核支付凭证中'"
 							:arrow="project_detail.status==2" class="cell"
 							@click="project_detail.status==2?uploadPaying():''"></u-cell-item>
 					</u-cell-group>

 				</view>
 				<view class="date" v-if="buttonShow">
 					<u-icon size="50rpx" color="#fce100" name="warning" style="margin-right: 10rpx; float: left">
 					</u-icon>
 					<view style="font-size: 30rpx; float: left;margin-top: 8rpx;">请在{{project_detail.end_time}}前完成操作
 					</view>
 				</view>
 			</view>
 		</view>
 		<view class="pro_bt">
 			<u-row style="margin: 50rpx;">
 				<u-col span="6">
 					<u-cell-item class="u-cell" :border-bottom="false" bg-color="#ffffff" :arrow="false"
 						:title="party+'详情'" :label="'查看'+party+'信息'" :title-style="titlestyle"
 						:label-style="labelstyle" center @click="clickCompanyInfo()">
 						<u-icon name="bangonglou" custom-prefix="custom-icon" slot="right-icon" size="70"></u-icon>
 					</u-cell-item>
 				</u-col>
 				<u-col span="6">
 					<u-cell-item class="u-cell" :border-bottom="false" bg-color="#ffffff" :arrow="false" title="咨询需求"
 						label="查看需求详情" :title-style="titlestyle" :label-style="labelstyle" center
 						@click="clcikDemandInfo()">
 			...
 			<u-row>
 				<u-col span="6">
 					<u-cell-item class="u-cell" :border-bottom="false" bg-color="#ffffff" :arrow="false" title="服务方案"
 						label="查看服务方案" :title-style="titlestyle" :label-style="labelstyle" center
 						@click="clickServerInfo()">
 						<u-icon name="xinhao" custom-prefix="custom-icon" slot="right-icon" size="50"></u-icon>
 					</u-cell-item>
 				</u-col>
 				<u-col span="6">
 					<u-cell-item class="u-cell" :border-bottom="false" bg-color="#ffffff" :arrow="false" title="服务时间"
 						:label="project_detail.commit_time+'至'+project_detail.end_time"
 						:title-style="{'font-size':'26rpx','font-weight':'bold','font-family':'微软雅黑','padding-top':'30rpx'}"
 						:label-style="labelstyle" center>
 						<u-icon name="clock" slot="right-icon" size="70"></u-icon>
 					</u-cell-item>
 				</u-col>
 			</u-row>
 			<view class="bt" v-if="buttonShow">
 				<button class="button" type="default" style="background-color: #1aad19;" hover-class="s-bg-light"
 					@click="clickButton($event,1)"><text class="txt">同意</text></button>
 				<button class="button" type="default" style="background-color: #c64128;" hover-class="w-bg-light"
 					@click="clickButton($event,2)"><text class="txt">拒绝并申述</text></button>
 			</view>
 		</view>
 		<u-toast ref="uToast" />
 ...
 <script>
 	...
 	export default {
 		...
 		methods: {
 								...
 								await getProject(params, this.project_detail.id).then(res => {
 									if (res.data.code === 1) {
 										...
 										if (res.data.data.proj_detail.status != 1) {
 											this.project_detail = res.data.data
 												.proj_detail
 											this.project_detail.commit_time =
 												formateDate(this.project_detail
 													.commit_time)
 											this.project_detail.create_time =
 												formateDate(this.project_detail
 													.create_time)
 											this.project_detail.end_time =
 												formateDate(this.project_detail
 													.end_time)
 										}
 									}
 			...
 			uploadPaying() {
 				uni.navigateTo({
 					url: "../paying/paying?id=" + this.project_detail.id + "&status=" + this.project_detail
 						.status + " &url=" + encodeURIComponent(JSON.stringify(this.project_detail
 							.pay_picture_url))
 				})
 			}
 		},
